...
class robot:
  ...
  def __init__(self, btdev, isAutoConnect = True):
    self.isConnected = False 
    self.btdev = btdev   
    #self.gatt = pexpect.spawn('gatttool -I -t random -b {0}'.format(btdev))
    if isAutoConnect:
       self.conn(self.btdev)

  def conn(self, btdev):
    self.gatt = pexpect.spawn('gatttool -I -t random -b {0}'.format(self.btdev))
    time.sleep(1)
    self.gatt.sendline('connect')
    self.gatt.expect('Connection successful')
    self.isConnected = True
    self.startConnectManager()
  ...
  def startConnectManager(self):
    thread.start_new_thread(self.connectManager, ())

  def connectManager(self):
    while self.isConnected:
       dataLine = self.gatt.readline()
       if '(gatttool:5934): GLib-WARNING **: Invalid file descriptor.' in dataLine:
          self.disconnect()
